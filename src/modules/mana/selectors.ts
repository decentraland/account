...
export const getTransactionsByType = (state: RootState, address: string, type: string): Transaction[] =>
  getTransactionsData(state).filter(tx => isEqual(tx.from, address) && tx.actionType === type)
...
export const getPurchases = (state: RootState) => getData(state).purchases.filter(isManaPurchase)

export const getWalletDeposits = createSelector<RootState, Deposit[], string | undefined, Deposit[]>(
  getDeposits,
  getAddress,
  (deposits, address) => (address ? deposits.filter(deposit => deposit.from.toLowerCase() === address.toLowerCase()) : [])
)

export const getWalletWithdrawals = createSelector<RootState, Withdrawal[], string | undefined, Withdrawal[]>(
  getWithdrawals,
  getAddress,
  (withdrawals, address) => (address ? withdrawals.filter(withdrawal => withdrawal.from.toLowerCase() === address.toLowerCase()) : [])
)
...
>(
  ...
  (transactions, deposits, withdrawals, purchases, walletAddress) => {
    ...
    for (const tx of transactions.filter(tx => tx.from.toLowerCase() === walletAddress?.toLowerCase())) {
      const { network } = getChainConfiguration(tx.chainId)
      const deposit = deposits.find(deposit => tx.hash === deposit.hash)
      const withdrawal = withdrawals.find(withdrawal => tx.hash === withdrawal.initializeHash)
      if (deposit) {
        const accountTransaction: AccountTransaction<Deposit> = {
          hash: tx.hash,
          type: TransactionType.DEPOSIT,
          status: deposit.status === DepositStatus.PENDING ? AccountTransactionStatus.PENDING : mapStatus(tx.status),
          data: deposit,
        ...
        if (tx.actionType === TRANSFER_MANA_SUCCESS && walletAddress && tx.from === walletAddress) {
    ...
    for (const purchase of purchases.filter(purchase => purchase.address.toLowerCase() === walletAddress?.toLowerCase())) {
      const accountTransaction: AccountTransaction<Purchase> = {
        ...
        status:
          ...
          (purchase.status === PurchaseStatus.PENDING && purchase.timestamp + ONE_HOUR < Date.now())
    ...
    result[Network.MATIC].sort((t1, t2) => t2.data.timestamp - t1.data.timestamp)
    result[Network.ETHEREUM].sort((t1, t2) => t2.data.timestamp - t1.data.timestamp)

    return result
  }
)

export const getWithdrawalImportError = createSelector<RootState, string | null, string | null>(getError, error => {
  if (!error) {
    return null
  }

  const unformatedImportError = error.startsWith(IMPORT_WITHDRAWAL_REQUEST) ? error.split(' - ')[1] : null
