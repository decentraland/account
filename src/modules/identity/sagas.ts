...
function* handleGenerateIdentityRequest(action: GenerateIdentityRequestAction) {
  ...
  try {
    ...
    const expiration = Number(
      env.get('REACT_APP_IDENTITY_EXPIRATION_MINUTES', ONE_MONTH_IN_MINUTES)
    )
...
function* handleLogin(action: LoginRequestAction) {
  ...
  if (shouldLogin && !restoreSession) {
    ...
    if (shouldConnectWallet) {
      ...
      const enableWallet: Race<
        EnableWalletSuccessAction,
        EnableWalletFailureAction
      > = yield takeRace(ENABLE_WALLET_SUCCESS, ENABLE_WALLET_FAILURE)
      ...
      const connectWallet: Race<
        EnableWalletSuccessAction,
        EnableWalletFailureAction
      > = yield takeRace(CONNECT_WALLET_SUCCESS, CONNECT_WALLET_FAILURE)
    ...
    if (!identity) {
      ...
      const generateIdentity: Race<
        GenerateIdentitySuccessAction,
        GenerateIdentityFailureAction
      > = yield takeRace(GENERATE_IDENTITY_SUCCESS, GENERATE_IDENTITY_FAILURE)
  ...
  if (wallet && identity) {
    ...
    yield put(
      loginFailure(
        restoreSession ? 'Failed to restore session' : 'Failed to login'
      )
    )
  }
}
